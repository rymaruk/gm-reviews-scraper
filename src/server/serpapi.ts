import { getSerpapiKeys } from './env'
import type { ParsedMapsLink } from './maps-url'
...
async function serpapiSearch(params: SerpSearchParams): Promise<Record<string, unknown>> {
  ...
  for (const [index, apiKey] of keys.entries()) {
    try {
      return await serpapiSearchWithKey(params, apiKey)
    } catch (error) {
      lastError = error instanceof Error ? error : new Error('SerpAPI request failed.')
...
async function serpapiSearchWithKey(
